...
In the same year, Google famously told users: ``We're working on Linux support--hang tight!''.\cite{google_drive_for_linux_is_on_the_way} The announcement slowly went under the radar and the project was eventually cancelled. This gave birth to an infamous joke website\cite{how_long_since_google_said_hang_tight} which counts how much time users have been waiting for the Linux client.

In \date{September 2017}, Google made another announcement regarding the desktop application. The official Drive app would be discontinued in \date{March 2018}, its place being taken by the new \emph{Backup and Sync} app.\cite{introducing_backup_and_sync}\cite{google_drive_is_being_replaced_by_backup_and_sync}
