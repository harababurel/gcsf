...
\multicolumn{1}{ |l| }{}             &
\multicolumn{1}{  l| }{Commits}      & 120               & 395                & 511               \\ \cline{2-5}
\multicolumn{1}{ |l| }{}             &
\multicolumn{1}{  l| }{Releases}     & 1                 & 23                 & 71                \\ \cline{2-5}
\multicolumn{1}{ |l| }{}             &
\multicolumn{1}{  l| }{Contributors} & 1                 & 7                  & 12                \\ \cline{2-5}
\multicolumn{1}{ |l| }{}             &
\multicolumn{1}{  l| }{Stars}        & 8                 & 489                & 2063              \\ \cline{2-5}
\multicolumn{1}{ |l| }{}             &
...
\begin{lstlisting}[frame=single,caption=Creating a virtual environment and installing GDriveFS]
...
\begin{lstlisting}[frame=single,caption=GDriveFS nonexistent authentication command]
...
Unless stated otherwise, all file systems were mounted using their default configuration. All results reported in the following sections are averaged among 10 different executions. The test account has been artificially populated with 2000 files and 100 directories, totalling 2.8 GB. The maximum depth of any file in the the file tree is 4.
...
One of the first operations performed after mounting the file system is in many cases a file listing. Whether this is achieved using a command line tool such as \codeword{ls} or a GUI file explorer, the file system has to respond to the same system calls: \codeword{readdir} and \codeword{lookup}. For this benchmark, I decided to measure the execution time of the \codeword{tree} command \cite{tree_man_page}. This  command explores an entire directory recursively, constructing an ASCII tree-like structure of all files and directories.
...
\begin{tabular}{|r|l|}
\cline{1-2}
File size (MB)   & Checksum errors (\%)   \\ \cline{1-2}
1                & 20                     \\ \cline{1-2}
10               & 40                     \\ \cline{1-2}
50               & 90                     \\ \cline{1-2}
100              & 80                     \\ \cline{1-2}
...
\caption{Checksum errors reported when reading files from GDriveFS} \label{tab:checksum_errors_fresh_read}
\end{table}
...
\caption{Checksum errors reported when reading cached files from GDriveFS} \label{tab:checksum_errors_cached_read}
\end{table}
